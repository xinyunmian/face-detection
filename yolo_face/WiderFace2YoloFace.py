    ...
    def __init__(self, txt_path, preproc=None):
        ...
        for line in lines:
            ...
            if line.startswith('#'):
                ...
                path = txt_path.replace('train_resize.txt', 'images/') + dirname + "/" + imgname
...
def wider2yolo(labp):
    aa = WiderFaceDetection(labp)

    for i in range(len(aa.imgs_path)):
        # print(i, aa.imgs_path[i])
        imgpath = aa.imgs_path[i]
        imgdir, hz = imgpath.split(".")
        img = cv2.imread(imgpath)
        save_txt_path = imgdir + ".txt"
        with open(save_txt_path, "w") as f:
            ...
            for idx, label in enumerate(labels):
                ...
                minwh = min(label[2], label[3])
                if minwh < 5 or label[2] > width or label[3] > height or label[19] == 0:
                    continue
                ...
                for i in range(len(annotation[0])):
                    str_label = str_label + " " + str(np.around(annotation[0][i], 8))
                str_label = str_label.replace('[', '').replace(']', '')
                str_label = str_label.replace(',', '') + '\n'
                f.write(str_label)

def widerface2yolotxt(save_path, labp):
    aa = WiderFaceDetection(labp)
    ...
    for i in range(len(aa.imgs_path)):
        ...
        with open(save_txt_path, "w") as f:
            ...
            for idx, label in enumerate(labels):
                ...
                minwh = min(label[2], label[3])
                if minwh < 5 or label[2] > width or label[3] > height:
                    continue
...
if __name__ == "__main__":
    save_img = 'D:/data/imgs/widerface_clean/train'
    label_path = "D:/data/imgs/widerface_clean/train_resize.txt"
    # widerface2yolotxt(save_img, label_path)
    wider2yolo(label_path)